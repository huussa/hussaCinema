...
export default function Screens() {
    ...
    api
      ...
      .then(async (data) => {
        ...
        const results = await Promise.allSettled(
          list.map((screen) => api.getScreenSeats(screen.id)),
        );
        const next = {};
        results.forEach((result, index) => {
          if (result.status === "fulfilled")
            next[list[index].id] = result.value.seats || [];
        });
        setSeatData(next);
      })
  ...
  return (
    <div>
      <PageHeader
        title="Screens"
        description="Monitor each screen and its current seat utilization."
      />
      {error && (
        <div className="mb-5 rounded-xl border border-red-900/40 bg-red-950/20 p-3 text-sm text-red-300">
          {error}
        </div>
      )}
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {metrics.map(({ screen, occupied, total, percent }) => (
          <article
            key={screen.id}
            className="rounded-[22px] border border-white/[.06] bg-[#110d0f] p-5 transition hover:border-red-900/30"
          >
            <div className="flex items-start justify-between gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="font-semibold">{screen.name}</h2>
            ...
            <div className="mt-7 flex items-end justify-between">
              ...
              <div className="flex items-center gap-1.5 text-xs text-zinc-600">
                <Armchair className="h-3.5 w-3.5" /> {occupied}/{total}
              </div>
            </div>
            ...
            <div className="mt-4 flex justify-between text-[11px] text-zinc-700">
              <span>Occupied</span>
              <span>Available {Math.max(total - occupied, 0)}</span>
            </div>
          </article>
        ))}
      </div>
      ...
      {!metrics.length && (
        <div className="rounded-2xl border border-white/[.06] bg-[#110d0f] p-12 text-center text-sm text-zinc-600">
          No screens found.
        </div>
      )}
    </div>
