...
export default function EditMovie({ params }) {
  ...
  useEffect(() => {
    api
      ...
      .then((d) => {
        setM(d.movie);
        setGenreIds((d.movie.genres || []).map((g) => g.id).join(","));
      })
  ...
  async function save(e) {
    ...
    setError("");
    try {
      await api.updateMovie(params.id, {
        title: m.title,
        description: m.description,
        posterUrl: m.posterUrl,
        trailerUrl: m.trailerUrl,
        duration: Number(m.duration),
        genreIds: genreIds.split(",").map(Number).filter(Boolean),
      });
  ...
  return (
    <div>
      <PageHeader
        title="Edit movie"
        description="Update the movie through PATCH /movies/:id."
      />
        ...
        <section className="rounded-2xl border border-white/[.06] bg-panel p-5 sm:p-7">
          <div className="grid gap-5 md:grid-cols-2">
            ...
            <Field
              ...
              type="number"
              value={m.duration}
              onChange={(e) => setM({ ...m, duration: e.target.value })}
            />
            ...
            <Field
              label="Poster URL"
              value={m.posterUrl || ""}
              onChange={(e) => setM({ ...m, posterUrl: e.target.value })}
            />
            <Field
              label="Trailer URL"
              value={m.trailerUrl || ""}
              onChange={(e) => setM({ ...m, trailerUrl: e.target.value })}
            />
            <Field
              label="Genre IDs"
              value={genreIds}
