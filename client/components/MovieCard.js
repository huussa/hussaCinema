        ...
        <div className="p-3.5">
          ...
          <div className="mt-2 flex items-center justify-between gap-2 text-[10px] text-zinc-600">
            <span className="flex items-center gap-1">
              <Clock3 className="h-3 w-3" /> {movie.duration} min
            </span>
            <span className="flex items-center gap-1 text-amber-400">
              <Star className="h-3 w-3 fill-current" /> Cinema
            </span>
          </div>
          <div className="mt-2 flex flex-wrap gap-1">
            {(movie.genres || []).slice(0, 2).map((genre) => (
              <span
                key={genre.id}
                className="rounded-full bg-white/[.04] px-2 py-1 text-[9px] text-zinc-600"
              >
                {genre.name}
              </span>
            ))}
