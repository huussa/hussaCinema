      ...
      <div className="flex h-full flex-col">
        ...
        <nav className="mt-6 space-y-1">
          ...
          {user?.role === "admin" && (
            <Link
              href="/admin"
              onClick={() => setOpen(false)}
              className="flex w-full items-center gap-3 rounded-xl px-3 py-3 text-sm font-medium text-zinc-500 hover:bg-white/[.04] hover:text-white"
            >
        ...
        <div className="mt-auto">
          {user ? (
            ...
            <Link
              href="/auth/login"
              className="flex w-full items-center gap-3 rounded-xl bg-red-600 px-3 py-3 text-sm font-semibold text-white hover:bg-red-500"
            >
              <UserRound className="h-4 w-4" />
              Sign in
            </Link>
  ...
  return (
    <>
      ...
      <button
        ...
        className="fixed left-4 top-4 z-40 rounded-xl border border-white/[.07] bg-[#0d0a0b]/90 p-3 text-zinc-300 shadow-xl backdrop-blur-lg lg:hidden"
      >
